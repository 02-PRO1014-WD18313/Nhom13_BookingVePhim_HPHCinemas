...
function InsertPhong($maphong, $trangthai){
    $sql = "INSERT INTO phong(maphong, soluongday, trangthai) VALUES ('{$maphong}',6,{$trangthai})";
    $id = inserted_id($sql);
    return $id;
}
...
function UpdatePhong($id,$maphong, $trangthai){
    $sql = "UPDATE phong SET maphong = '{$maphong}', soluongday = 6, , trangthai = $trangthai  WHERE id_phong = {$id}";
    execute($sql);
}
